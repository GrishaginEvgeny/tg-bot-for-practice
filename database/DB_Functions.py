        ...
        if (len(list) < 3):
            for l in list:
                if (URLFunctions.check_for_repos_owner(l)):
                    text += URLFunctions.get_rec_from_group(repos_url=l) + '\n'
                else:
                    text += URLFunctions.get_rec_from_users(repos_url=l) + '\n'
        else:
            for i in range(0, 3, 1):
                if (URLFunctions.check_for_repos_owner(list[i])):
                    text += URLFunctions.get_rec_from_group(repos_url=list[i]) + '\n'
                else:
                    text += URLFunctions.get_rec_from_users(repos_url=list[i]) + '\n'
